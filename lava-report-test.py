...
def test_api_post(config, build_id, test_results):
 # scan if we shall open a test_suite/test_set sequence
 ...
 # this is the bundle for one lava-command
 # we'll have the command, followed by the 
 # power stats
 #
 my_test_case = {}
 
 for test in test_results['test_results']:
     if not 'measurement' in test.keys():
        # 'PASS' is fine, but not 'pass'
        my_test_case['status'] = test['result'].upper()
        my_test_case['name'] = test['test_case_id']
        my_test_case['measurements'] = []
     else:
        my_measurement = {}
        my_measurement['name'] = test['test_case_id']
        my_measurement['units'] = test['units']
        my_measurement['measure'] = test['measurement']
        my_test_case['measurements'].append(my_measurement)

# lets use the canned form, and post everything together.
 test_cases = []
 test_cases.append(my_test_case)

 power_set = {}
 power_set['name'] = "power-set"
 power_set['test_case'] = test_cases

 test_suite = {}
 test_suite['name'] = "lava-command"
 test_suite['test_set'] = []
 test_suite['test_set'].append(power_set)
 test_suite['build_id'] = build_id
 test_suite['lab_name'] = config.get("lab")
 return test_suite
